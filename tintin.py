...
seasnum = pp1.replace("Season 0", "") # get season number from folder selection

ss = seas.details("1570",seasnum) # get tmdb details for that season of spongebob
...
for l in ss.episodes: # for each entry in the episodes for that season of bob l'eponge
    ...
    renamed = f"{epname} S{ssnum:02}E{epnum:02}" # text model for renaming
    properEPdict[f"The Adventures of Tintin S{ssnum:02}E{epnum:02}"] = renamed   # in the dictionary for the tmdb side of comparing,
...
for filename in os.listdir(path):    # for each episode in the season:
    ...
    if os.path.isfile(f) and seasStr in ff: # lose 24 more brain cells; run through if file and if wrong format is detected
        ...
        ep = ff[0:]  # remove first 35 characters of filename (because they were named "SpongeBob Squarepants - Season x - episodename.avi")
        print(ep)
        epfile = f"{path}/{filename}" # setup file name to match episode name
        print(epfile)
        epdict[f"{ep}"] = epfile  # file name match to episode name
...
for key in sortedProperDict.keys():
    ...
    if maxMatchRatio >= 0.95 and maxMatchKey in epdict.keys():
